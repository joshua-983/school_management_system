...
class Assignment(models.Model):
    ...
    def has_valid_attachment(self):
        """
        Check if assignment has a valid attachment (not empty/null)
        This fixes the Django ORM quirk where NULL displays as empty string
        """
        if not self.attachment:
            return False
        
        # Check if it has a name and the name is not empty
        return hasattr(self.attachment, 'name') and bool(self.attachment.name.strip())
    
    @property
    def attachment_display(self):
        """Get proper attachment display"""
        if not self.has_valid_attachment:
            return None
    ...
    def save(self, *args, **kwargs):
        ...
        # ===========================================
        # CRITICAL FIX: Fix Django's empty string issue for NULL FileFields
        # ===========================================
        if hasattr(self, 'attachment') and self.attachment == '':
...
class StudentAssignment(models.Model):
    ...
    def can_student_download_assignment(self):
        ...
        return self.assignment.has_valid_attachment
    ...
    def send_submission_notification(self):
        ...
        try:
            ...
            message_type = "Draft" if self.is_draft else "Submission"
            
            # Import here to avoid circular import
            from your_app.utils.notifications import create_notification
            
            create_notification(
                recipient=teacher,
                title=f"New Assignment {message_type}",
                message=f"{self.student.get_full_name()} has {'submitted a draft for' if self.is_draft else 'submitted work for'} '{self.assignment.title}'",
    ...
    def get_submission_quality(self):
        """Get submission quality based on score"""
    ...
    def get_time_spent_estimate(self):
        """Calculate estimated time spent based on submission patterns"""
        # This is a simplified estimate - in production, you'd track actual time
        if not self.submitted_date or not self.created_at:
            return None
        
        time_spent = self.submitted_date - self.created_at
    ...
    def get_priority_class(self):
        ...
        return priority_map.get(self.get_priority_level(), 'bg-secondary text-white')
    ...
    def get_rubric_scores_summary(self):
        """Get summary of rubric scores if available"""
        if not self.rubric_score:
            return None
        
        try:
            if isinstance(self.rubric_score, dict):
                total_points = sum(criterion.get('score', 0) for criterion in self.rubric_score.values())
                max_points = sum(criterion.get('max_score', 0) for criterion in self.rubric_score.values())
                
                return {
                    'total_points': total_points,
                    'max_points': max_points,
                    'percentage': (total_points / max_points * 100) if max_points > 0 else 0,
                    'criteria_count': len(self.rubric_score)
                }
        except Exception as e:
            logger.error(f"Error parsing rubric scores: {str(e)}")
        
        return None
    
    @property
    def is_urgent(self):
        """Check if this assignment is urgent for grading"""
        return self.get_priority_level() == 'urgent'
    ...
    def update_score(self, score, feedback="", teacher_comments="", feedback_document=None):
        ...
        self.send_grading_notification()
    
    def send_grading_notification(self):
        """Send notification to student when assignment is graded"""
        try:
            ...
            create_notification(
                ...
                message=f"Your assignment '{self.assignment.title}' has been graded. Score: {self.score}/{self.assignment.max_score}",
                notification_type="GRADE",
                link=reverse('student_assignment_detail', kwargs={'pk': self.pk})
            )
        except Exception as e:
            logger.error(f"Error sending grading notification: {str(e)}")
...
class Bill(models.Model):
    ...
    # FIXED: Use consistent statuses from constants
    status = models.CharField(
        max_length=20,  # Increased from 10 to accommodate 'refunded'
    ...
    def update_status(self):
        ...
        from .constants.financial import PAYMENT_TOLERANCE
        ...
        if abs(self.amount_paid - self.total_amount) <= PAYMENT_TOLERANCE:
            self.status = 'paid'
...
class Fee(models.Model):
    # FIXED: Use consistent statuses from constants
    PAYMENT_STATUS_CHOICES = FEE_STATUS_CHOICES  # Now consistent with Bill
    PAYMENT_MODE_CHOICES = PAYMENT_METHOD_CHOICES  # Consistent naming
    ...
    # FIXED: Now using the same statuses as Bill (just adding 'draft' and 'unpaid')
    payment_status = models.CharField(
        max_length=20, 
        choices=FEE_STATUS_CHOICES, 
        default='unpaid'  # Changed from 'issued' to 'unpaid' for new fees
    )
    ...
    def update_payment_status(self):
        ...
        from .constants.financial import PAYMENT_TOLERANCE, PAYMENT_GRACE_PERIOD
